...
import { SuccessState } from "@/features/auth/SuccessState";
import { validateRegisterForm } from "@/utils/register.validate";
...
export function RegisterView() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/auth/register`, {
        ...
        body: JSON.stringify({ fullName: form.fullName, username: form.username, email: form.email, password: form.password, birthDate: form.birthDate || null }),
      });
  ...
  if (success) return <SuccessState firstName={form.fullName.split(" ")[0]} />;

  return (
    <div className="min-h-screen flex items-center justify-center py-12"
      style={{ background: "radial-gradient(ellipse at 60% 20%, #2a1208 0%, #0d0704 60%, #000 100%)" }}>
      <Wrapper>
