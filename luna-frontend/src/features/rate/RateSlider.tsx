...
import { useRates } from "@/hooks/useData";

export const RateSlider = () => {
  const { data: reviews = [] } = useRates();

  return (
    <section className="w-full py-28 bg-black text-white text-center relative overflow-visible">
      <Wrapper>
        <motion.h4
        ...
        <Swiper
          ...
          navigation={{ prevEl: ".rate-prev", nextEl: ".rate-next" }}
        ...
        >
          {reviews.map((review) => (
            <SwiperSlide
              key={review.id}
              className="transition-transform duration-500 ease-in-out scale-90 opacity-70 [&.swiper-slide-active]:scale-110 [&.swiper-slide-active]:opacity-100 [&.swiper-slide-active]:shadow-2xl"
            >
              <motion.article
                ...
                viewport={{ once: true }}
                whileHover={{ y: -6 }}
                className="bg-[#e7e4e2] text-black rounded-3xl px-8 py-10 h-full flex flex-col items-center gap-4"
              >
                <div>
